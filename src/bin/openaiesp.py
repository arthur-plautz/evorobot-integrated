...
class Algo(EvoAlgo):
    ...
    def reset_env(self, salt=1):
        self.policy.env.seed(self.seed + (self.cgen * self.batchSize)  + (self.id * self.n_worker_samples) + salt)
        ...
        if self.rank == 0:                                      # Master: postevaluate the best sample of last generation
            ...
            if self.bestsol is not None:
                ...
                for t in range(self.policy.nttrials):
                    ...
                    eval_rews, eval_length = self.policy.rollout(1, curriculum=None, seed=(self.seed + 100000 + t))
                    gfit += eval_rews               
                    ceval += eval_length
            ...
            for b in range(self.n_worker_samples):               # Worker (evaluate a fraction of the population)
                for bb in range(2):
                    ...
                    eval_rews, eval_length = self.policy.rollout(self.policy.ntrials, curriculum=self.curriculum, seed=(self.seed + (self.cgen * self.batchSize) + (self.id * self.n_worker_samples) + b))
    ...
    def run(self):
        ...
        while (self.steps < self.maxsteps):
            ...
            if self.rank == 0:
                ...
                print(
                    ...
                    (self.seed, self.progress, self.cgen, self.steps / 1000000, self.bestfit, self.bestgfit, self.bfit, self.avgfit, self.avecenter)
                )
                self.save_summary()
        ...
        if self.rank == 0:
            self.save_all()                      # save data at the end of evolution
